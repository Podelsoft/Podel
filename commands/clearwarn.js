...
module.exports.run = async (bot, message, args) => {
  ...
  if (message.member.hasPermission('KICK_MEMBERS')) {
    ...
    let user = message.guild.member(bot.users.find(user => user.username.toLowerCase().includes(args.join(' ').toLowerCase()))) || message.mentions.members.first() || bot.users.find(user => user.id === args[0]);
