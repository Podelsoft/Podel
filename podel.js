...
bot.on("voiceStateUpdate", (oldState, newState) => {
  ...
  if (newState.channelID === null) {
    db.delete(`voiceState_${newState.id}`);
  } else {
    db.push(`voiceState_${newState.id}`, 'true');
  }
...
bot.on("message", async message => {
  ...
  if (message.channel.id === "696538508227248178") {
    ...
    if (message.guild.id === "696515024746709003") {
      ...
      let role3 = message.guild.roles.cache.find(role => role.id === "730938567337181266");
      let voiceStatus = db.fetch(`voiceState_${message.author.id}`);
      if (voiceStatus !== null) {
